...
public class NewCobbleNuker {
    ...
    public void onRender(RenderWorldLastEvent event) {
        ...
        if(crop != null) {
            RenderUtils.drawBlockBox(crop, new Color(128, 255, 0), Main.configFile.lineWidth, event.partialTicks);
        }
    }
    ...
    private BlockPos closestCrop() {
        ...
        if (playerPos != null) {
            switch (Main.configFile.farmNukeIndex) {
                case 0:
                    for (BlockPos blockPos : BlockPos.getAllInBox(playerPos.subtract(vec3i), playerPos.add(vec3i))) {
                        IBlockState blockState = Main.mc.theWorld.getBlockState(blockPos);
                        if (blockState.getBlock() == Blocks.nether_wart || blockState.getBlock() == Blocks.potatoes || blockState.getBlock() == Blocks.cobblestone || blockState.getBlock() == Blocks.carrots || blockState.getBlock() == Blocks.pumpkin || blockState.getBlock() == Blocks.melon_block || blockState.getBlock() == Blocks.brown_mushroom || blockState.getBlock() == Blocks.red_mushroom || blockState.getBlock() == Blocks.cocoa) {
                            if (Main.configFile.farmShapeIndex == 0) {
                                if (!broken.contains(blockPos) && isXZSameAsPlayer(blockPos)) {
    ...
    private boolean isXZSameAsPlayer(BlockPos blockPos){
        BlockPos playerPos = Main.mc.thePlayer.getPosition();
        playerPos = playerPos.add(-1, 0, -1);
        if(playerPos.getX() != blockPos.getX()) return false;
        if(playerPos.getZ() != blockPos.getZ()) return false;
        System.out.println(playerPos + " " + blockPos);
        return true;
    }
}
