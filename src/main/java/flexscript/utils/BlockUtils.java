...
public class BlockUtils {
    ...
    public static Block getFrontBlock(int blockCount){
        return getRelativeBlock(0, 0, blockCount);
    }


    public static Block getRelativeBlock(float x, float y, float z) {
        return (Main.mc.theWorld.getBlockState(
                new BlockPos(
                        Main.mc.thePlayer.posX + (getUnitX() * z) + (getUnitZ() * -1 * x),
                        Main.mc.thePlayer.posY + y,
                        Main.mc.thePlayer.posZ + (getUnitZ() * z) + (getUnitX() * x)
                )).getBlock());
    }
    ...
    public static int getUnitX() {
        double modYaw = (Main.mc.thePlayer.rotationYaw % 360 + 360) % 360;
